    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {(plans?.plans || []).map((plan: any) => {
          ...
          return (
            <div key={plan.name} className={`bg-[#1C1C1E] rounded-2xl p-5 border-2 ${isCurrent ? 'border-primary shadow-lg shadow-primary/10' : 'border-[#38383A]'}`}>
              ...
              <p className="text-2xl font-bold text-white mb-1">{plan.price === 0 ? 'FREE' : `${formatMMK(plan.price)} MMK`}</p>
              <p className="text-xs text-[#8E8E93] mb-4">{plan.price > 0 ? 'per month' : 'forever'}</p>
              <ul className="space-y-2">
                {(plan.features || []).map((f: string, i: number) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-[#8E8E93]">
      ...
      <div className="bg-[#1C1C1E] rounded-2xl border border-[#38383A] overflow-hidden">
        {history.length === 0 ? (
          <div className="text-center py-12 text-[#8E8E93]"><CreditCard className="w-10 h-10 mx-auto mb-2 opacity-30" /><p>No payments yet</p></div>
        ) : (
          <table className="w-full">
            <thead className="bg-[#2C2C2E]">
              <tr>
                <th className="text-left px-6 py-3 text-xs font-semibold text-[#8E8E93] uppercase">Date</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-[#8E8E93] uppercase">Plan</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-[#8E8E93] uppercase">Method</th>
                <th className="text-right px-6 py-3 text-xs font-semibold text-[#8E8E93] uppercase">Amount</th>
            ...
            <tbody className="divide-y divide-[#2C2C2E]">
              {history.map((p: any) => (
                <tr key={p.id} className="hover:bg-[#2C2C2E]/50">
                  <td className="px-6 py-4 text-sm text-[#8E8E93]">{new Date(p.created_at).toLocaleDateString()}</td>
                  <td className="px-6 py-4 font-medium text-white capitalize">{p.plan}</td>
                  <td className="px-6 py-4 text-sm text-[#8E8E93]">{p.payment_method}</td>
                  <td className="px-6 py-4 text-right font-semibold text-white">{formatMMK(p.amount)} MMK</td>
