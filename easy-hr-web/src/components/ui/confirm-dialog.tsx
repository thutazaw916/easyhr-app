...
const variantStyles = {
  danger: { icon: Trash2, iconBg: 'bg-red-500/15', iconColor: 'text-red-400', btnBg: 'bg-red-500 hover:bg-red-600' },
  warning: { icon: AlertTriangle, iconBg: 'bg-amber-500/15', iconColor: 'text-amber-400', btnBg: 'bg-amber-500 hover:bg-amber-600' },
  info: { icon: AlertTriangle, iconBg: 'bg-blue-500/15', iconColor: 'text-blue-400', btnBg: 'bg-primary hover:bg-primary-700' },
};
    ...
    <ConfirmContext.Provider value={{ confirm: confirmFn, prompt: promptFn }}>
      ...
      {state && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[90] p-4" onClick={() => handleClose(state.type === 'confirm' ? false : null)}>
          <div className="bg-[#1C1C1E] rounded-2xl w-full max-w-sm shadow-2xl border border-[#38383A]" onClick={e => e.stopPropagation()}>
            <div className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-bold text-white">{state.options.title}</h3>
                  <p className="text-sm text-[#8E8E93] mt-1">{state.options.message}</p>
                  {state.type === 'prompt' && (
                    <input
                      autoFocus
                      value={promptValue}
                      ...
                      className="mt-3 w-full px-4 py-2.5 rounded-xl border border-[#38383A] bg-[#2C2C2E] text-white placeholder-[#8E8E93] outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary text-sm"
                    />
                  )}
                </div>
              </div>
            ...
            <div className="flex gap-3 px-6 pb-6">
              <button onClick={() => handleClose(state.type === 'confirm' ? false : null)}
                className="flex-1 py-2.5 rounded-xl border border-[#38383A] text-[#8E8E93] hover:bg-[#2C2C2E] font-medium text-sm transition">
